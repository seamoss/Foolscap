...
/* electron-builder infers a GitHub publish provider from GH_TOKEN /
 * GITHUB_TOKEN in the environment even when the config has no publish
 * block, and that inference is what writes app-update.yml into the
 * bundle. The Store build runs with every such token withheld. */
const PUBLISH_TOKENS = ['GH_TOKEN', 'GITHUB_TOKEN', 'GITLAB_TOKEN', 'KEYGEN_TOKEN', 'BITBUCKET_TOKEN', 'BT_TOKEN']
const run = (cmd, args, env = {}) => {
  const clean = { ...process.env, ...env }
  for (const name of PUBLISH_TOKENS) delete clean[name]
  execFileSync(cmd, args, { cwd: root, stdio: 'inherit', env: clean })
}
const fail = (msg) => {
  console.error(`\ndist-store: ${msg}`)
  process.exit(1)
...
/* electron-builder writes app-update.yml for any publish configuration it
 * can find — the config's block, an environment token, or, failing both,
 * the GitHub remote of this very repository. Only an explicit top-level
 * `publish: null` switches that off (a per-target null is dropped by its
 * option merging). The Store build therefore runs from a derived config,
 * generated fresh from electron-builder.yml every time, with exactly that. */
const yaml = require(require.resolve('js-yaml', { paths: [path.dirname(require.resolve('electron-builder/package.json'))] }))
const config = yaml.load(fs.readFileSync(path.join(root, 'electron-builder.yml'), 'utf8'))
config.publish = null
config.buildVersion = buildNumber
fs.mkdirSync(path.join(root, 'dist'), { recursive: true })
const storeConfig = path.join(root, 'dist/electron-builder.store.yml')
fs.writeFileSync(storeConfig, '# Generated by scripts/dist-store.cjs — do not edit.\n' + yaml.dump(config))
/* The packaged package.json would otherwise still list electron-updater
 * under dependencies — a declaration, not code, but a string a reviewer
 * can grep, and extraMetadata cannot override that field. The Store app
 * has no runtime dependencies at all, so package.json goes into the
 * packager without the field and comes back byte for byte. */
const pkgPath = path.join(root, 'package.json')
const originalPkg = fs.readFileSync(pkgPath, 'utf8')
const stripped = { ...pkg }
delete stripped.dependencies
fs.writeFileSync(pkgPath, JSON.stringify(stripped, null, 2) + '\n')
try {
  run('pnpm', ['exec', 'electron-builder', '--config', storeConfig, '--mac', 'mas', '--universal', '--publish', 'never'])
} finally {
...
console.log(`\ndist-store: clean — no updater, no feed file, no node_modules; ${pkg.version}-mas (${bundleVersion})`)
for (const f of pkgs) console.log(`dist-store: ${path.join('dist/mas-universal', f)}`)

/* The submission's mark in history: an annotated tag on this commit,
 * v0.16.0-mas.1 (the release workflow ignores -mas tags). Only on a clean
 * tree — a pkg built from uncommitted changes has no commit to mark. */
const tag = `v${pkg.version}-mas.${submission}`
const dirty = execFileSync('git', ['status', '--porcelain'], { cwd: root }).toString().trim()
if (dirty) {
  console.log(`dist-store: working tree not clean — not tagging ${tag}`)
} else {
  const exists = execFileSync('git', ['tag', '-l', tag], { cwd: root }).toString().trim()
  if (exists) console.log(`dist-store: tag ${tag} already exists`)
  else {
    execFileSync('git', ['tag', '-a', tag, '-m', `Mac App Store submission: ${pkg.version} build ${buildNumber}`], { cwd: root })
    console.log(`dist-store: tagged ${tag} (push with: git push origin ${tag})`)
  }
}
