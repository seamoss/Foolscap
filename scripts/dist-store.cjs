...
/* electron-builder writes app-update.yml whenever the config has a publish
 * block — a per-target `publish: null` doesn't stop it. So the Store build
 * runs from a derived config with the block removed, generated fresh from
 * electron-builder.yml every time (one source of truth). */
